...
<div class="row">
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                <form action="{{ route('Worker.update','test') }}" method="post" autocomplete="off" enctype="multipart/form-data">
                    @method('patch')
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="name" value="{{$worker->name}}" id="name" class="form-control" required>
                        </div>
                        <div class="col-md-4">
                            <label for="birth-date" class="mr-sm-2">Birth Date :</label>
                            <input type="date" id="birth" name="birth" value="{{$worker->birth_date}}" class="form-control" required>
                        </div>
                        <div class="col-md-4">
                            <label for="hiring-date" class="mr-sm-2">Hiring Date :</label>
                            <input type="date" id="hiring" name="hiring" value="{{$worker->hiring_date}}" class="form-control" required>
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <input type="number" name="number" value="{{$worker->number_phone}}" id="number-phone" class="form-control" required>
                        </div>
                        <div class="col-md-8">
                            <label for="email" class="mr-sm-2">Email :</label>
                            <input type="email" id="email" name="email" value="{{$worker->email}}" class="form-control" required>
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        <div class="col">
                            ...
                            <select name="gender" id="gender" class="custom-select my-1 mr-sm-2" title="select gender" required>
                                <option value="{{$worker->gender->id}}">{{$worker->gender->name}}</option>
